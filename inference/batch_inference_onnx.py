...
class BatchInferenceOnnx(InferenceBase):
    ...
    def _setup(self):
        if torch.cuda.is_available():
            self.device = torch.device(f"cuda:{self.cfg.inference.gpu_id}")
            log.info(f"onnx runtime session is using GPU {self.cfg.inference.gpu_id}")
        else:
            self.cfg.inference.gpu_id = -1
            self.device = torch.device("cpu")
            log.info("onnx runtime session is using CPU")
    ...
    def infer(self, bdy_swap_method: dict | None = None):
        ...
        for batch_id, (input, target) in enumerate(data_loader):
            ...
            for step in trange(predict_iters, desc=f"Infer batch {batch_id}"):
                ...
                assert type(inp_upper) is np.ndarray and type(inp_surface) is np.ndarray
    ...
    def unload_model(self) -> None:
        """Manually release ONNXRuntime session to free CPU/GPU memory."""
        sess = getattr(self, "ort_sess", None)
        if sess is None:
            return
        ...
        # force Python GC so ORT session destructor runs ASAP
        gc.collect()

        # optional: clear PyTorch CUDA cache (won't free ORT allocations, but helps overall)
