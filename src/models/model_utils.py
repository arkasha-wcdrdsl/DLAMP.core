...
def get_scheduler_with_warmup(
    ...
    def linear_decay(current_epoch):
        ...
        return max(lr_scale, 0.05)

    match schedule_type:
        case "cosine":
            return LambdaLR(optimizer, lr_lambda=cosine_decay, last_epoch=last_epoch)
        case "constant":
            return LambdaLR(optimizer, lr_lambda=constant, last_epoch=last_epoch)
        case "constant_warmup":
            return LambdaLR(optimizer, lr_lambda=constant_warmup, last_epoch=last_epoch)
        case "linear":
            return LambdaLR(optimizer, lr_lambda=linear_decay, last_epoch=last_epoch)
        case "linear_decay":
            return LambdaLR(optimizer, lr_lambda=linear_decay, last_epoch=last_epoch)
        case _:
