...
class PanguModel(nn.Module):
    ...
    def __init__(
        ...
        window_size: list[tuple[int, int, int]],
        upper_levels: int,
        upper_channels: int,
        ...
        post_process_size: int | None = None,
    ) -> None:
        ...
        image_shape_3d: tuple[int, int, int] = (upper_levels, *image_shape)
        inp_Z, inp_H, inp_W = [ceil(x / y) for x, y in zip(image_shape_3d, patch_size)]
        inp_Z += 1  # surface
        drop_path_list = np.linspace(0, max_drop_path_ratio, sum(depths)).tolist()
        embed_dim_config: list[int] = [(2**i) * embed_dim for i in range(self.hierarchy)]
        ...
        if post_process_size is not None:
            self.refine_block = RefineBlock_cnn(
                post_process_size=post_process_size,
                upper_channels=7,      # 7
                surface_channels=8     # 8
            )
        else:
            self.refine_block = Identity()
    ...
    def forward(
        ...
        # )
        output_upper, output_surface = self.refine_block(
            output_upper, output_surface
        )
...
class RefineBlock_cnn(nn.Module):
    def __init__(
        self,
        post_process_size: int = 3,
        upper_channels: int = 7,
        surface_channels: int = 8,
    ) -> None:

        super().__init__()

        self.refine_upper = nn.Conv3d(
            in_channels=upper_channels,
            out_channels=upper_channels,
            kernel_size=(1, post_process_size, post_process_size),
            stride=(1, 1, 1),
            padding=(0, post_process_size // 2, post_process_size // 2),
            groups=upper_channels,
        )

        self.refine_sfc = nn.Conv3d(
            in_channels=surface_channels,
            out_channels=surface_channels,
            kernel_size=(1, post_process_size, post_process_size),
            stride=(1, 1, 1),
            padding=(0, post_process_size // 2, post_process_size // 2),
            groups=surface_channels,
        )
    def forward(
        self, x_upper: torch.Tensor, x_surface: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        x_upper = self.refine_upper(x_upper)
        x_surface = self.refine_sfc(x_surface)
...
class PatchEmbedding(nn.Module):
    def __init__(
        ...
        self.register_buffer("topography_mask", torch.Tensor(topography_mask))
        ...
        if not is_divisible_elementwise(list(img_shape), list(patch_size)):
            log.warning(
    ...
    def forward(
        ...
        if self.land_mask is not None and self.topography_mask is not None:
            ...
            input_surface = torch.cat(
                tuple([
                    input_surface,
                    torch.Tensor(repeat(self.land_mask, "h w -> b 1 h w 1", b=batch_size)),
                    torch.Tensor(repeat(self.topography_mask, "h w -> b 1 h w 1", b=batch_size)),
                ]),
                dim=-1,
            )
