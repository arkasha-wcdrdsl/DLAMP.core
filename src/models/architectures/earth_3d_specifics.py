...
class EarthSpecificBlock(nn.Module):
    def __init__(
        ...
        Returns:
            None
        """
        assert is_divisible_elementwise(list(input_shape), list(window_size)), (
...
class ConvProjection3D(nn.Module):
    def __init__(self, dim, window_size, kernel_size=3):
        super().__init__()
        self.wZ, self.wH, self.wW = window_size

        # Depthwise Separable Convolution
        # groups=dim makes it Depthwise
        # Use GroupNorm for Transformer stability(LayerNorm will make the memory explode)
        self.conv = nn.Sequential(
            nn.Conv3d(dim, dim, kernel_size=kernel_size, padding=kernel_size//2, groups=dim, bias=False),
            nn.GroupNorm(num_groups=1, num_channels=dim),
            nn.Conv3d(dim, dim, kernel_size=1, bias=True)
        )

    def forward(self, x):
        # x: (BN, wP, dim) -> BN = B*mZ*mH*mW, wP = wZ*wH*wW
        BN, wP, C = x.shape

        # 1. Reshape to 3D Window: (BN, C, wZ, wH, wW)
        x = x.transpose(1, 2).reshape(BN, C, self.wZ, self.wH, self.wW)

        # 2. Apply Spatial-Vertical Convolution
        x = self.conv(x)

        # 3. Flatten back: (BN, wP, C)
        x = x.reshape(BN, C, wP).transpose(1, 2)
        return x
...
class EarthAttention3D(nn.Module):
    def __init__(
        ...
        # self.q_proj = ConvProjection3D(dim, window_size)
        # self.k_proj = ConvProjection3D(dim, window_size)
        # self.v_proj = ConvProjection3D(dim, window_size)
    ...
    def forward(
        ...
        # # --- Modified block start: use ConvProjection3D for q, k, and v separately. ---
        # B_N, wP, C = x.shape
        # h = self.num_head
        # d_h = C // h

        # # Reshape after ConvProjection3D to match the original attention shape:
        # # (B*num_windows, num_head, win_Z*win_H*win_W, dim//num_head).
        # query = self.q_proj(x).reshape(B_N, wP, h, d_h).permute(0, 2, 1, 3)
        # key   = self.k_proj(x).reshape(B_N, wP, h, d_h).permute(0, 2, 1, 3)
        # value = self.v_proj(x).reshape(B_N, wP, h, d_h).permute(0, 2, 1, 3)
        # # --- Modified block end. ---

        attention = (query @ key.transpose(-2, -1)) / self.scale

        assert isinstance(self.position_index, (torch.Tensor, FakeTensor))
