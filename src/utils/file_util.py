    ...
    match DATA_SOURCE:
        case "NEO171_RWRF":
            if isinstance(data_compose, DataCompose):
                file_name = gen_path(target_time, use_Kth_hour_pred, data_compose)
                return read_cwa_npfile(file_name, data_compose.is_radar, dtype)
            elif isinstance(data_compose, list):
                ret = {}
                for ele in data_compose:
                    file_name = gen_path(target_time, use_Kth_hour_pred, ele)
                    ret[str(ele)] = read_cwa_npfile(file_name, ele.is_radar, dtype)
                return ret
            else:
                raise TypeError(
                    "For NEO171_RWRF, data_compose must be a DataCompose or a list of DataCompose"
                )
        case "CWA_RWRF" | "TRIMED_RWRF" | "TRIMED_RWRF_MUL1000":
            file_name = gen_path(target_time, use_Kth_hour_pred)
            # return test_file_exists(file_name, data_compose, dtype)
...
def read_cwa_ncfile(
    ...
    def fn(dc: DataCompose):
        ...
        """
        if dc.var_name == DataType.Qw and DATA_SOURCE=='CWA_RWRF':
            # Qw = Qr + Qc + Qi + Qs + Qg
            ...
            if data == 0:
                raise ValueError("All components of Qw are empty.")
            data *= 1000  # kg/kg -> g/kg
        elif dc.var_name == DataType.Qw and DATA_SOURCE=='TRIMED_RWRF_MUL1000':
            data = dataset[dc.combined_key].values * 1000  # kg/kg -> g/kg
        else:
    ...
    try:
        if isinstance(data_compose, DataCompose):
            ...
            raise TypeError(
                "data_compose that passed to read_cwa_ncfile must be a DataCompose or a list of DataCompose"
            )
    except:
...
def test_file_exists(
    ...
    FOR TESTING
    """
    assert file_path.exists(), f"file {file_path} doesn't exist"
    def fn(dc: DataCompose):
        if dc.level.is_surface():
            data = np.zeros((1,224,224))
        else:
            data = np.zeros((1,13,224,224))
        ...
        if dc.level.is_surface():
            ...
            pres_lvs = np.array([1000, 925, 850, 700, 600, 500, 400, 300, 250, 200, 150, 100, 50])
            (idx,) = np.where(pres_lvs == int(dc.level.nc_key))
            return data[0, idx[0]]
    ...
    match data_source:
        ...
        case "TRIMED_RWRF" | "TRIMED_RWRF_MUL1000":
            ...
            return (
                Path(DATA_PATH)
                / f"{target_time.strftime('%Y-%m-%d_%H')}"
                / f"wrfout_d01_{predict_dt.strftime('%Y-%m-%d_%H')}_interp.nc"
            )
        case _:
...
def convert_hydra_dir_to_timestamp(hydra_dir: Path | str) -> str:
    ...
    """
    try:
        assert isinstance(hydra_dir, Path)
