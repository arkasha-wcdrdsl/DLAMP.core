...
def gen_TW_only_terrain():
    ...
    target_lat = data_gnrt.yield_data(start_t, dc_lat, use_Kth_hour_pred=0)
    target_lon = data_gnrt.yield_data(start_t, dc_lon, use_Kth_hour_pred=0)

    # Taiwan-only terrain data
    # OBJECTID_1	OBJECTID	townname	countyname	BASIN_NAME	N_1	E_1	elevation	slope	aspect	Shape_Leng	ORIG_FID	geometry
...
def extract_landmask_from_ncfile():
    ...
    start_t = datetime.strptime(data_config["start_time"], data_config["format"])
    filename = gen_path(start_t, use_Kth_hour_pred=0)
    ...
    # terrain_mask = terrain[::6, ::6]  # (75, 75)
    # landsea_mask = landsea[::6, ::6]  # (75, 75)
    terrain = terrain[25:-25, 25:-25]  # (400, 400)
    landsea = landsea[25:-25, 25:-25]  # (400, 400)
    terrain_mask = terrain[::5, ::5]  # (224, 224)
    landsea_mask = landsea[::5, ::5]  # (224, 224)
