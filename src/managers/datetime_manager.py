...
class DatetimeManager:
    ...
    def __init__(
        ...
        output_itv: dict[str, int] = {"hours": 1},
        sequence_mode: bool = False,
    ):
        self.start_time = datetime.strptime(start_time, format)
        self.end_time = datetime.strptime(end_time, format)
        self.interval = timedelta(**interval)
        self.output_itv = timedelta(**output_itv)
        self.forecast_steps: int = self.output_itv // self.interval
        self.format = format
    ...
    def build_initial_time_list(
        self, data_list: list[DataCompose], use_Kth_hour_pred: int, quick: bool = False
    ) -> DatetimeManager:
        # always check files if not specified, quick build is not reliable, not worth it when training
        if quick and Path(BLACKLIST_PATH).exists():
    ...
    def _build_init_time_list(
        ...
        use_Kth_hour_pred: int,
        save_output: bool = False,
    ) -> None:
        """
        Builds initial time list based on the start time and end time. Eliminates datetime which
        does not meet the sanity check for both current time and next time (output time).
        Save all missing current time to blacklist.
        ...
        checked_queue: list[datetime|None] = []
        checking_time = self.start_time
        while checking_time < self.end_time:
            if self.sanity_check(checking_time, data_list, use_Kth_hour_pred):
                checked_queue.append(checking_time)
            else:
                checked_queue.append(None)
                missing.append(checking_time)
            if len(checked_queue) > self.forecast_steps:
                if None not in checked_queue or (not self.sequence_mode and None not in [checked_queue[0], checked_queue[-1]]):
                    assert checked_queue[0], "checked_queue[0] should not be None in this condition."
                    self.time_list.append(checked_queue[0])
                checked_queue.pop(0)

            pbar.update(1)
            checking_time += self.interval
        pbar.close()
        ...
        log.info(f"Removed {num_time-len(self.time_list)} datetimes during data sanity check.")
    ...
    def _quick_build_init_time_list(self) -> None:
        ...
        blacklist are excluded from the time list. Also checks next_time.
        """
        s = time.time()
        ...
        with tqdm(total=num_time) as pbar:
            pbar.set_description("Quick Building initial time list...")
            init_time = self.start_time
            while init_time < self.end_time:
                target_time = init_time + self.interval * self.forecast_steps
                times_to_check = set([init_time, target_time])
                if self.sequence_mode:
                    # generate a new list contains all times between init_time, target_time
                    checking_time = init_time + self.interval
                    while checking_time < target_time:
                        times_to_check.add(checking_time)
                        checking_time += self.interval

                if not times_to_check & missing:
                    self.time_list.append(init_time)
                pbar.update(1)
                init_time += self.interval
        pbar.close()
    ...
    def random_split(
        self, ratios: list[float | int], split_method: str = "random") -> DatetimeManager:
        """
        ...
        if split_method == "random":
            ...
            train_end = int(num_groups * ratios_frac[0])
            valid_end = int(num_groups * (ratios_frac[0] + ratios_frac[1]))
    ...
    def sanity_check(
        ...
        match data_source:
            case "NEO171_RWRF":
                data_filename_generator = (gen_path(dt, use_Kth_hour_pred, data) for data in data_list)
                while True:
                    try:
            ...
            case "CWA_RWRF":
                ...
                data_filename = gen_path(dt, use_Kth_hour_pred)
                return True if data_filename.exists() else False
            case "TRIMED_RWRF" | "TRIMED_RWRF_MUL1000":
                # for trimed version of cwa wrf
                data_filename = gen_path(dt, use_Kth_hour_pred)
                res = True if data_filename.exists() else False
                if res:
                    return True
                else:
                    log.debug(f"{data_filename} doesn't exist")
                    return False
                # return True if data_filename.exists() else False
            case _:
