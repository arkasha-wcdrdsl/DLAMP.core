...
class CustomDataset(Dataset):
    def __init__(
        ...
        data_interval: dict[str, int] = {"hours": 1},
        model_native_interval: dict[str, int] = {"hours": 1},
    ):
        super().__init__()
        self._ilen = inp_len
        self._olen = oup_len
        ...
        # sequence mode, if True, the dataset will return a sequence of outputs with interval of `model_native_interval` until `oup_itv`, instead of only one output at `oup_itv`
        self.sequence_mode = sequence_mode
        self.step_itv = timedelta(**model_native_interval)
        if sequence_mode:
            assert (timedelta(**data_interval) % self.step_itv).total_seconds() == 0, "model_native_interval not compatetble with data_interval"
            assert (self._oitv % self.step_itv).total_seconds() == 0, "oup_itv not compatetble with model_native_interval"

        # Filter out blacklisted times from init_time_list
        self._init_time_list = self._filter_blacklist(init_time_list)
    ...
    def __getitem__(self, index) -> tuple[dict[str, np.ndarray], ...]:
        """
        ...
        Returns:
            tuple: A tuple containing the input and output data, or None if data is unavailable.
        """
        if self._is_train_or_valid:
            index *= self._sr
        ...
        if self.sequence_mode:
            middle_time_steps = [input_time + i * self.step_itv for i in range(1, int(self._oitv / self.step_itv))]
        ...
        try:
            ...
            middle_steps = []
            if self.sequence_mode:
                middle_steps = [self._get_variables_from_dt(t, is_input=True) for t in middle_time_steps]
            output = self._get_variables_from_dt(output_time, is_input=False)
            return input, *middle_steps, output
        except (FileNotFoundError, RuntimeError, OSError) as e:
            # If files are missing, raise error
            raise Exception(f"Failed to load data for timestamp {input_time}") from e

    def _load_blacklist(self) -> None:
        """
        Load blacklist from BLACKLIST_PATH into memory.
        """
                    ...
                    for line in f:
                        ...
                        if dt_str:
                            try:
                                ...
                                self._blacklist.add(dt)
                            except ValueError:
                                log.warning(f"Failed to parse blacklist entry: {dt_str}")
            except Exception as e:
                log.warning(f"Failed to load blacklist: {e}")
        else:
            log.info(f"Blacklist file not found at {BLACKLIST_PATH}")

    def _filter_blacklist(self, init_time_list: list[datetime]) -> list[datetime]:
        """
        Filter out times that are in the blacklist from the init_time_list.

        Parameters:
            init_time_list (list[datetime]): The original time list.

        Returns:
            list[datetime]: Filtered time list with blacklisted times removed.
        """
        filtered_list = [dt for dt in init_time_list if dt not in self._blacklist]

        num_removed = len(init_time_list) - len(filtered_list)
        if num_removed > 0:
            log.info(f"Removed {num_removed} blacklisted timestamps from init_time_list")

        return filtered_list

    def _add_to_blacklist(self, dt: datetime) -> None:
        """
        Add a datetime to the blacklist and update the blacklist file.

        Parameters:
            dt (datetime): The datetime to add to the blacklist.
        """
        if dt not in self._blacklist:
            self._blacklist.add(dt)

            # Append to blacklist file
            try:
                with open(BLACKLIST_PATH, "a") as f:
                    f.write(dt.strftime(self._datetime_format) + "\n")
                log.info(f"Added {dt} to blacklist")
            except Exception as e:
    ...
    def _get_variables_from_dt(
        ...
        for key, value in output.items():
            ...
            if is_input and key == "surface" and self.add_time_features:
                ...
                stack_data = np.concatenate([stack_data, time_features], axis=-1)
